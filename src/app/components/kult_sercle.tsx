...
float Burisaba(vec2 pos, float power){
    float ret = 0.0 + power / Bu(pos) + power / Ri(pos + vec2(-3.0, 0.0)) + power / To(pos + vec2(-6.0, 0.0)) + power / Ba(pos + vec2(-9.0, 0.0));
    return ret;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    + smoothLine(len, 8.0, 0.02) + smoothLine(len, 7.5, 0.02) 
    + BurisabaCircle2(uv, 1.1, 8.0, 64.0, 12.0, 4.0, 7.5, 7.9, 5.0 
    + iTime * 0.7) 
    + BurisabaCircle2(uv, 1.2, 8.0, 64.0, 12.0, 4.0, 7.5, 7.9, 15.0 
    + iTime * 0.1564) 
    + BurisabaCircle2(uv, 1.45, 8.0, 64.0, 12.0, 4.0, 7.5, 7.9, 15.0 
    + iTime * 0.2418654) 
    ...
    + 0.1 / abs(boxLength(rotate(uv, M_PI * 2.0 * 0.6666) * vec2(8.0, 0.5) - vec2(0.0, 2.9)) - 1.0)) * mask 
    + circleTriangle(uv) 
    + c2.y 
    + c3.y 
    + c4.y;
...
export default function KultSercle() {
  ...
  useEffect(() => {
    ...
    function render() {
      if (!canvas || !gl || !positionBuffer || !program || startTimeRef.current === null) return;
      resize();
